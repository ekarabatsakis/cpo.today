...
export async function countryPage(cc, ctx) {
  ...
  const opRows = ((ops && ops.operators) || []).slice(0, 40).map((o) => `<tr><td>${esc(o.name)}</td><td class="num">${int(o.locations)}</td><td class="num">${int(o.evses)}</td><td class="num">${int(o.dc_evses)}</td>${live ? `<td class="num">${pct(o.avail_pct)}</td><td class="num">${pct(o.charging_pct)}</td><td class="num">${pct(o.down_pct)}</td>` : ""}<td class="num">${o.median_kwh_ac != null ? Number(o.median_kwh_ac).toFixed(2) : "–"}</td><td class="num">${o.median_kwh_dc != null ? Number(o.median_kwh_dc).toFixed(2) : "–"}</td></tr>`).join("");
...
<h2>Operators</h2>
<table class="t"><thead><tr><th>Operator</th><th class="num">Locations</th><th class="num">Charge points</th><th class="num">DC</th>${live ? `<th class="num">Available</th><th class="num">In use</th><th class="num">Down</th>` : ""}<th class="num">AC price/kWh</th><th class="num">DC price/kWh</th></tr></thead><tbody>${opRows}</tbody></table>
