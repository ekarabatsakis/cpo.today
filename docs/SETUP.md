...
- **Upstream outage**: the pipeline retries transient errors three times within the tick, keeps the last good data, records a warning in `<cc>/meta.json`, and the site header turns amber after 35 minutes without a new status file. A registry that is unreachable stays a notice (run green) while the last good snapshot is younger than three refresh intervals (at least 45 minutes for live feeds, at least 12 hours for inventories); only after that does the run fail and email you.
- **Inventory-only registries** (France, Germany, Cyprus) are checked on their own cadence (`refresh_minutes`, stored as `next_check` in `meta.json`), not every 10 minutes; a failed check is retried after an hour.
