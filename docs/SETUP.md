...
- **Upstream outage**: the pipeline retries transient errors three times within the tick, keeps the last good data, records a warning in `<cc>/meta.json`, and the site header turns amber after 35 minutes without a new status file. A registry problem never fails the 10-minute run (that would email on every tick): it is a notice while the last good snapshot is fresh and a warning once it is stale. The daily **Data health** workflow (`.github/workflows/health.yml`) fails, once a day, while any country's published data is older than its limit (3 h for live feeds, about a day for inventories), so a real outage produces one email a day.
- **Retiring a country**: remove it from `sources/__init__.py`; the next run's `cpo_pipeline.retire` step deletes its directory from the data branch and drops it from `index.json`, so it disappears from the site.
- **Inventory-only registries** (France, Cyprus) are checked on their own cadence (`refresh_minutes`, stored as `next_check` in `meta.json`), not every 10 minutes; a failed check is retried after an hour.
